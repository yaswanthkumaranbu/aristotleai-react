          ...
          <div className="card shadow">
            ...
            <div className="card-body">
              {changes.map((change, index) => (
                <div key={index} className="accordion mb-3" id={`accordion-${index}`}>
                  <div className="accordion-item">
                    <h2 className="accordion-header" id={`heading-${index}`}>
                      <button
                        className={`accordion-button ${selectedVersion === index ? '' : 'collapsed'}`}
                        type="button"
                        onClick={() => setSelectedVersion(selectedVersion === index ? null : index)}
                        aria-expanded={selectedVersion === index ? 'true' : 'false'}
                        aria-controls={`collapse-${index}`}
                      >
                        <span className="badge bg-secondary me-2">Version {change.version}</span>
                        {change.date}
                      </button>
                    </h2>
                    <div
                      id={`collapse-${index}`}
                      className={`accordion-collapse collapse ${selectedVersion === index ? 'show' : ''}`}
                      aria-labelledby={`heading-${index}`}
                      data-bs-parent={`#accordion-${index}`}
                    >
                      <div className={`accordion-body border border-dark p-3 ${selectedVersion === index ? 'fade-in' : ''}`}>
                        {selectedVersion === index && change.descriptions.map((desc, descIndex) => (
                          <div key={descIndex}>
                              ...
                              {desc.content.map((item, itemIndex) => (
                                <li key={itemIndex}>
                                  ...
                                  {item.includes(':') ? (
                                    <>
                                      {item.split(':')[0]}: 
                                      <span className="text-danger">{item.split(':')[1]}</span>
                                    </>
                                  ) : (
                                    item
                                  )}
